...
## SEB skills at school

::: {style="font-size:25px;"}
|                 | Self-management | Innovation | Social engagement | Cooperation | Emotional resilience |
|------------|:----------:|:----------:|:----------:|:----------:|:----------:|
| Achievement     |      **+**      |   **+**    |                   |             |                      |
| Engagement      |      **+**      |   **+**    |       **+**       |             |                      |
| Social outcomes |                 |            |       **+**       |    **+**    |                      |
...
These are linked to students' academic achievement, school engagement, social relationships, and well-being [@soto2024GoingTraitsSocial; @ringwald2025MoreSkillTrait; @feraco2025SocialEmotionalBehavioral; @napolitano2025ChangesSocialEmotional].
:::

# Who is a successful student?

From a personality perspective, we would say a student who has high openness and conscientiousness [@poropat2009MetaanalysisFivefactorModel; @chen2025BigFivePersonality].

From a skill perspective, we may say something very similar: high self-management and innovation
...
```         
Responses where screened for careless responding using multiple detection methods.
```
:::

<!-- --------------------------------------------------------------------- -->

# Study 1 @feraco2025SoftSkillsHigh
...
(Figure3 <- ggplot(res1.1, aes(x = est.std, y = rhs)) +
    ...
    geom_point(data = res02.1, aes(x = est.std, y = rhs, color = selfeff))
)
...
(Figure3 <- ggplot(res1.1, aes(x = est.std, y = rhs)) +
    ...
    theme(axis.text.x = element_text(angle = 45, vjust = 1.2, hjust=1, size = 14),
          legend.position = "none") +
    geom_point(data = res2.1, aes(x = est.std, y = rhs, color = engagement))
)
...
url <- "https://feracotommaso.github.io/Slides/SEB_SRL/ISSID/ISSID_seb_srl_slides.html"
